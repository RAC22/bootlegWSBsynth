...
let emoteExpression =
	/!+[\[]+(img)+[\]]+[(]+(emote\|)+(\w*)+[\|]+(\d*)+[\)]/gims
let otherEmote = /!+(\[gif]\(emote\|free_emotes_pack\|)+(\w*\))/gims
let carrot = /\^/g
...
function play() {
	...
	fetch(wsbUrl)
		...
		.then((res) => {
			...
			wsbDDthread = new Request(
				res.data.children[dailydiscussionidx].data.url + ".json"
			)
			...
			fetch(wsbDDthread)
				...
				.then((res) => {
					...
					for (let i = bufferNumber; i > 0; i--) {
						...
						let commenturl =
							"http://www.reddit.com" + res[1].data.children[i].data.permalink
...
async function readIt(phrase, author, commentLink) {
	...
	msg.onstart = function () {
		console.log(debugCounter + " of " + bufferNumber)
	}
